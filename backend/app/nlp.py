...
# Greeting / filler phrases to strip before parsing.
# These are common conversational openers that are NOT tasks.
...
# Chunks that consist entirely of filler words (no actionable content).
...
_VOICE_PRIORITY_MAP: dict[str, int] = {}
for _prefix, _val in [("низк", 1), ("средн", 2), ("высок", 3)]:
    for _suffix in ["ий", "ая", "ое", "яя", "ее", ""]:
        _VOICE_PRIORITY_MAP[_prefix + _suffix] = _val
...
def _extract_time_range(text: str) -> tuple[time | None, str | None, str]:
    """Extract time range like 'с 9 до 13'. Returns (start_time, end_desc, cleaned)."""
...
def parse_ru(
    ...
    # Extract voice priority before main parsing
    priority, text = _extract_voice_priority(text)

    # Extract time range (с 9 до 13) before main parsing
