...
_GREETING_PATTERNS: list[re.Pattern[str]] = [
    ...
    re.compile(
        ...
        r"ладно|окей|ок|давай|так|ну\s+так|ну|типа|как\s+бы|"
        r"э+|м+|это\s+самое|в\s+общем|собственно)\b[,.\s]*",
        re.IGNORECASE,
    ),
    re.compile(
...
_RE_MULTI_SPLIT = re.compile(
    ...
    r"а\s+ещ[её]|плюс|кроме\s+того|ну\s+и|и\s+потом|а\s+потом|"
    r"ну\s+ещ[её]|а\s+также|и\s+плюс|ещ[её]\s+надо|ещ[её]\s+нужно)\b[,.]?\s*)",
    re.IGNORECASE,
)
